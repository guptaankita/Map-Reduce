...
final_Filter_on_flight_date = FILTER filtered_on_join_records BY (monthf1 >=6 AND monthf1<=12 AND yearf1==2007) OR (monthf1>=1 AND monthf1<=5 AND yearf1 ==2008);

--compute the delay for each pair S1 and S2 in Flights1 and Flights2 respectively
averageDelay = FOREACH final_Filter_on_flight_date GENERATE arrDelayminsf1 + arrDelayminsf2 as val;
...
--store the total average delay in a file Output_join_version2
STORE totalaverageDelay INTO 's3://ankitamr/Output_join_version2';