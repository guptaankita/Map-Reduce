...
join_records = JOIN filterFlight1 BY (flightDatef1,destf1), filterFlight2 BY (flightDatef2,originf2);
...
finalFilter_on_F1 = FILTER filtered_on_join_records BY (monthf1 >=6 AND monthf1<=12 AND yearf1==2007) OR (monthf1>=1 AND monthf1<=5 AND yearf1 ==2008);

--now filter the records check that the flight date of Flights2 is in range
finalFilter_on_F2 = FILTER finalFilter_on_F1 BY (monthf2 >=6 AND monthf2<=12 AND yearf2==2007) OR (monthf2>=1 AND monthf2<=5 AND yearf2 ==2008);

--compute the delay for each pair S1 and S2 in Flights1 and Flights2 respectively
averageDelay = FOREACH finalFilter_on_F2 GENERATE arrDelayminsf1 + arrDelayminsf2 as val;
...
--store the total average delay in a file Output_join_version1
STORE totalaverageDelay INTO 's3://ankitamr/Output_join_version1';
